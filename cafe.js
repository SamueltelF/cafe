...
const { axios, Boom, cheerio, crypto, cfonts, colors, fs, ffmpeg, fetch, ms, moment, number, nodecache, request, readline, pino, util, os, exec, spawn, execSync } = require('./connect.js');

const { banner1, banner2 } = require('./arquivos/banner.js');
...
if (pairingCode && !cafe.authState.creds.registered) {
let number = await question(`${colors.cyan("\n• Exemplo do número para conectar: +5578998999989")}${colors.yellow("\n• Insira o número de telefone: ")}`);
number = number.replace(/[^0-9]/g, "");
let code = await cafe.requestPairingCode(number);
code = code?.match(/.{1,4}/g)?.join("-") || code;
console.log(`${colors.yellow("\n• Código para conectar ao WhatsApp: ")}` + colors.white(code));
rl.close();
}
cafe.ev.process(async(events) => {

if (events["messages.upsert"]) {
var upsert = events["messages.upsert"]
require("./index.js")(upsert, cafe)}
...
await cafe.sendPresenceUpdate("available");
console.log(colors.red(`CONECTADO COM SUCESSO...`))
await cafe.updateProfileStatus(`já estou conectado`)
break;
...
require("./index.js")(cafe, pastacode);
});
}
